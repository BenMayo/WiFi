#This code converts BNG to WGS84 (new)
from scipy import *
import sys
...
cur.execute("SELECT * FROM grid WHERE (minLat IS NULL) LIMIT 1",)
    
for i in range(1):
    row1 = cur.fetchone()
    minEast = Decimal(row1[1])
    maxEast = Decimal(row1[3])
    minNorth = Decimal(row1[2])
    maxNorth = Decimal(row1[4])
    rowId = int (row1[0])
...
lat, lon = OSGB36toWGS84(int(minEast), int(minNorth))
...
cur.execute("UPDATE grid SET minLat= %s, minLon=%s WHERE id = %s ;",(lat, lon, rowId,))
